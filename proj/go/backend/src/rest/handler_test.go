	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if response.StatusCode != tt.outStatusCode {
				t.Errorf("Received Status code %d does not match expected status code %d",
					response.StatusCode, tt.outStatusCode)
			}
			...
			if !reflect.DeepEqual(respBody, tt.expectedRespBody) {
				...
				t.Errorf("Received HTTP response body %+v does not match expected HTTP response Body %+v",
					respBody, tt.expectedRespBody)
			}
		})
	}
}
